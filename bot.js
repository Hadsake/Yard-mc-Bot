...
client.on('message', message => {
  ...
  if (command === 'tavsiyeni-gönder' || command === 'tavsiye') {
     let str = '<327058504194195457>';//@silmeying?
    let id = str.replace(/[<@!>]/g, '');
    let mesaj = args.slice(0).join(' ');
    if (mesaj.length < 1) return message.reply(` ⚠ tavsiyeni yazmayı unuttun. ⚠ `);
    message.channel.sendEmbed(new Discord.RichEmbed()
    .setColor('RANDOM')
    .setDescription(':white_check_mark: Tavsiye bildirildig? Tavsiyenizi bildirdiğiniz için teşekkür ederiz!'));
    const embed = new Discord.RichEmbed()
    .setColor('RANDOM')
...
client.on('guildCreate', guild => {
    let channel = bot.channels.get("327058504194195457")
     ...
     client.on("message", message => {
    const dmchannel = client.channels.find("name", "deneme");
...
client.on('ready', () => {
  client.setInterval(() => {
      client.user.setActivity("g?davet | g?yardım | Yardımcıbot ", { type: "STREAMING" },{ status: "idle" });
      client.user.setActivity(` g?destek | g?davet`, { type: "STREAMING" },{ status: "idle" });
...
client.on('ready', () => {
 client.setInterval(() => {
      client.user.setGame(`Yeni Komutlar : g?reklamtaraması - g?roller`, 'https://www.twitch.tv/ibrahim12655');
      client.user.setGame(`g?yardım - g?davet | Site yakında`, 'https://www.twitch.tv/ibrahim12655');
  }, 5000); // Burada 7000 yazan yeri degistirirseniz oynuyor kisminin degisme hizi suresi degisecektir 7000 7 saniye anlamina gelmektedir bunu 5000 yaparsaniz 5 saniye seklinde ayarlayabilirsiniz.
});
